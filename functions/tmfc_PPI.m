...
for iSub = start_sub:nSub
    ...
    switch tmfc.defaults.parallel
        case 0                              % Sequential
            ...
            try                                                             % Updating the TMFC GUI window
                main_GUI = guidata(findobj('Tag','TMFC_GUI'));              % Finding the GUI's object via handle
                set(main_GUI.TMFC_GUI_S4,'String', strcat(num2str(iSub), '/', num2str(nSub), ' done'),'ForegroundColor',[0.219, 0.341, 0.137]);    % Assigning the status to the TMFC varaible
            end
        case 1                              % Parallel
            try 
                send(D,[]);
