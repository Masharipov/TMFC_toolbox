function varargout = LSS_GUI_BIG(ALL_CONDS)
    ...
    if ~isempty(ALL_CONDS)
        ...
        for i = 1:length(ALL_CONDS)
            LST_1 = vertcat(LST_1, ALL_CONDS(i).list_name);        
        end
        ALL_CONDS_COPY = ALL_CONDS;
    end 
    catch
    LST_1 = {};
    end
    ...
    function action_5(~,~)
       ...
       if isempty(LST_2)
           ...
           LS_GR.LSS_residual_ts.conditions = EXPORT;
           close(LSS_GUI);
           assignin("base", 'tmfc', LS_GR);
           warning(length(LST_2)+" areas successfully selected");
       end
       
       GDR = evalin("base", 'tmfc');
       if isstruct(GDR.LSS_residual_ts.conditions)
            ...
            for i_STO = 1:D(1) % Subject number
                ...
                Ringer.subjects(i_STO).LSS_residual_ts = I;
