function varargout = TMFC()
    
% Opens main GUI window
%
% An tmfc structure contains the following structures:
%    
...
%   
%   tmfc.subjects.paths: paths individual subjects' SPM.mat files
...
%
%   tmfc.ROI_set:         information about selected ROI set
...
%% ======================[ TMFC Settings Action ]==========================
    ...
    function Settings(ButtonH, EventData, MAIN_F)
        ...
        MAIN_F_SET = figure("Name", "Settings", "NumberTitle", "off", "Units", "normalized", "Position", [0.62 0.26 0.205 0.575],'MenuBar', 'none','ToolBar', 'none','color','w','Resize','off','WindowStyle','modal',"Tag", "MAIN_WINDOWS_Settings");
        ...
        set(MF_S_OK , 'callback', @OK_SYNC);   
            ...
            %    set([handles.SUB,handles.SUB_stat,handles.FIR_TR, handles.FIR_TR_stat, handles.LSS_R, handles.LSS_R_stat, handles.ROI, handles.ROI_stat, handles.BSC, handles.gppi, handles.save_p, handles.open_p, handles.change_p, handles.settings, handles.bgrd],'fontunits','normalized', 'fontSize', SIZE);
            %end
...
%% ----------- ---------- TMFC SELECT SUBJECTS Action --------- -----------  
    ...
    function ROI_EXEC(ButtonH, EventData, MAIN_F)
        ...
        if R_checker.ROIs(1).paths == ""
            tmfc_select_ROIs_GUI();
            
            SPD_ROI = evalin('base', 'tmfc');
            if SPD_ROI.ROIs(1).paths == ""
                warning("No ROIs selected");
            else
            tmfc_ROI_SET();
            end
        else
            % ARe you sure you want to CLEAR WHATEVER IS INSIDE..& re-do
            tmfc_select_ROIs_GUI();
            tmfc_ROI_SET();
        end
...
%retriveal of data from GUI_1
        %GDP = findobj("Tag", "MAIN_WINDOWS");
        %BB = getappdata(GDP,'Addresses');
        %set(handles.text3,"String", BB);
        %THE_DETAILS = {};
        %assignin('base','THE_DETAILS',BB);
        %disp(THE_DETAILS);
